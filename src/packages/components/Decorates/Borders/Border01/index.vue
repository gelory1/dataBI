...
export default {
  ...
  mounted() {
    if (this.chartConfig && this.chartConfig.option) {
      this.colors = this.chartConfig.option.colors || []
      this.dur = this.chartConfig.option.dur || ''
      this.backgroundColor = this.chartConfig.option.backgroundColor || ''
    }
  }
}
</script>
