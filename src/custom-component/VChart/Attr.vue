      ...
      <el-form-item>
        <el-dropdown>
          <span class="el-dropdown-link">
            更换图表类型<i class="el-icon-arrow-down el-icon--right" />
          </span>
    ...
    methods: {
        openStaticWinbox() {
            ...
            this.$nextTick(() => {
                ...
                // 初始化图表数据在editor中
                const data = JSON.stringify(this.curComponent.propValue.option.series.data)
                const xAxis = JSON.stringify(this.curComponent.propValue.option.xAxis.data)
        ...
        updatedata() {
            ...
            const Arrdata = this.findstring(str, '[', ']')
            const ArrXAxis = this.findstring(str.substr(str.indexOf(']') + 1), '[', ']')
